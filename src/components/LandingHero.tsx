...
export default function LandingHero({ onNavigate }: LandingHeroProps) {
  ...
  const [inquiryInterest, setInquiryInterest] = useState("Computing & Software Development");
  ...
  const handleInquirySubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("https://formspree.io/f/xrewwknz", {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
        body: JSON.stringify({
          name: inquiryName,
          phone: inquiryPhone,
          academicInterest: inquiryInterest,
          source: "hero-callback-request"
        })
      });
      ...
      setTimeout(() => {
        setInquiryName("");
        setInquiryPhone("");
        setInquiryInterest("Computing & Software Development");
              ...
              <form onSubmit={handleInquirySubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-900 border border-slate-700 rounded-lg py-2.5 px-3.5 text-white text-sm focus:outline-none focus:border-blue-500 transition-colors"
                  >
                ...
                <button
                  ...
                  className="w-full bg-blue-500 hover:bg-blue-600 disabled:bg-slate-700 disabled:cursor-not-allowed text-white py-3 rounded-lg font-bold text-sm shadow-md transition-colors cursor-pointer"
                >
                  {inquirySubmitting ? "Submitting..." : "Book Callback Now"}
                </button>
              </form>
            )}
          </div>
